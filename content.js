    ...
    try {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (window.wordwatchHighlightMode === 'pos') {
          ...
          // JLPT level highlighting using basic_form for more accurate matching
          const word = token.basic_form; // Use basic_form (dictionary form) if available
          const wordEntry = jlptWordDatabase[word];
          
          console.log('Checking JLPT level for word:', word, 'wordEntry:', wordEntry);
          console.log(window.wordwatchJlptLevels);
          if (wordEntry && window.wordwatchJlptLevels.includes(wordEntry.level.toString())) {
            console.log("setting highlightClass to pos-jlpt");
            highlightClass = `pos-jlpt`;
          }
        }
